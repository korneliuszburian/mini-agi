...
The default tracker is local markdown: one file per ticket under
`.scratch/<feature-slug>/issues/<NN>-<slug>.md`. If the repo has a real
tracker configured (see the repo docs), publish there instead and apply the
...
passes a reference (a spec path, an issue number or URL) as an argument,
...
green. Don't force it into a tracer bullet; sequence it as
**expand–contract**. First expand: add the new form beside the old so
nothing breaks. Then migrate the call sites over in batches sized by blast
radius (per package, per directory), each batch its own ticket blocked by
the expand, keeping CI green batch to batch because the old form still
exists. Finally contract: delete the old form once no caller remains, in a
ticket blocked by every migrate batch. When even the batches can't stay
green alone, keep the sequence but let them share an integration branch
that all block a final integrate-and-verify ticket — green is promised only
there.
...
- **A real issue tracker (GitHub, Linear, …)** → publish one issue per
  ...
  / sub-issue relationship where it has one; otherwise set each ticket's
  "Blocked by" to the blocking issues. Apply the `ready-for-agent` triage
  label unless instructed otherwise — the tickets are agent-grabbable by
  construction.
...
A reference to the parent issue on the tracker (if the source was an
...
more precisely than prose can (state machine, reducer, schema, type
shape), inline it and note briefly that it came from a prototype. Trim to
the decision-rich parts — not a working demo, just the important bits.

## Completion criteria

- [ ] Every ticket is a tracer-bullet vertical slice with a demoable end
      state; no horizontal slices.
- [ ] Blocking edges are explicit and correct; a ticket's blockers
      genuinely gate it.
- [ ] The breakdown was quizzed with the user and approved before
      publishing.
- [ ] One file/issue per ticket, numbered in dependency order.
- [ ] No file paths or code snippets in tickets outside the documented
      exception.
- [ ] Frontier (unblocked tickets) is identifiable from the published
      tickets alone.
