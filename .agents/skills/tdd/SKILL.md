...
(`memory/derived/context-brief.md`) so test names and interface vocabulary
match the project's domain language, and respect ADRs in the area you're
touching.
...
a specification — "user can checkout with valid cart" tells you exactly
what capability exists — and survives refactors because it doesn't care
...
unconfirmed seam. You can't test everything — agreeing the seams up front
...
- [ ] The seams under test were written down and confirmed before any test.
- [ ] Every test was red first (failure output quoted), then green.
- [ ] No test recomputes its expected value from the code under test.
- [ ] Each cycle touched one seam, one test, one minimal implementation.
- [ ] The full suite ran at the end; output quoted.
