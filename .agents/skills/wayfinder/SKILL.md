...
the signal you've reached the edge of the map and it's time to hand off. An
...
vanish — a name wraps its link — but they ride *inside* the name, never
stand in for it.
...
physically live is tracker-specific.** The default tracker is local
...
`prototype`, `decision-review`, `task` (see [Ticket
Types](#ticket-types)).

A session **claims** a ticket by assigning it to the dev driving the map,
**first**, before any work, so concurrent sessions skip it. That assignee
_is_ the claim: an open, unassigned ticket is unclaimed.
...
human sees what's takeable without opening the map. Only a tracker that
lacks native blocking falls back to a body convention. A ticket is
...
for the human's side of it (an agent that answers its own interview
...
- **Research** (AFK): Reading documentation, third-party APIs, or local
  ...
  Resolved by a **research subagent**. Use when knowledge outside the
...
- **Prototype** (HITL): Raise the fidelity of the discussion by making a
  ...
  stub, or UI/logic code. Links the prototype as an asset. Use when "how
...
- **Decision-review** (HITL): A conversation with the user, one question at
  a time, to pin down what they want — never inventing their side of the
  decision. The default case.
- **Task** (HITL or AFK): Manual work that must happen before a *decision*
  ...
  seen. This is the one type that *does* rather than decides — and it earns
  its place by unblocking a decision, not by delivering the destination.
  The agent drives it alone where it can (AFK); otherwise it hands the
  human a precise checklist (HITL). Resolved when the work is done; the
  answer records what was done and any resulting facts (credentials
...
scope, just not sharp enough to ticket. Write as loosely or as fully as the
...
belong in **Not yet specified**. It gets its own **Out of scope** section
on the map: work you've consciously ruled out of _this_ effort. Scope, not
...
1. **Name the destination.** Interview the user directly, one question at a
   time, to pin down what this map is finding its way to — the spec,
   decision, or change. The destination fixes the scope, so it's settled
   first.
2. **Map the frontier.** Interview again, **breadth-first** this time: fan
   out across the whole space rather than deep on any one thread,
   surfacing the open decisions and the first steps takeable now. **If this
   surfaces no fog** — the way to the destination is already clear, the
   whole journey small enough for one session — you don't need a map. Stop
...
4. **Create the tickets you can specify now** as child issues of the map —
   then wire blocking edges in a **second pass** (issues need ids before
   they can reference each other). Wiring sorts them into the frontier and
   the blocked; everything you can't yet specify stays in the fog — the
...
5. Add newly-surfaced tickets (create-then-wire); graduate any fog the
   ...
   **rule it out of scope** rather than resolving it on the route. If the
   decision invalidates other parts of the map, update or delete those
   tickets.
...
- [ ] Decisions-so-far, Not-yet-specified, and Out-of-scope sections are
      kept distinct; out-of-scope work never graduates.
