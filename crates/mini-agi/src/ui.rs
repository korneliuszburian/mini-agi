    ...
    function renderRuns(data) {
      ...
      function emitRows(list) {
        list.forEach(function (row) {
          ...
          if (row.verification.target_missing) html += '<p class="row-subtitle">Verifier target is missing from disk — verification cannot run; the claim stays unverified.</p>';
          else html += commandHtml(row.verification.command_text, row.verification.execute_path, "This executes the run-declared verifier command in its declared target and writes attribution evidence. Continue?") + '</div></details></td>';
