...
fn tool_definitions() -> Vec<Value> {
    ...
    TOOLS
        ...
        .map(|t| {
            ...
            let property_schema = |ty: &str| json!({ "type": ty });
            for name in t.required {
                properties[name] = property_schema("string");
            }
            json!({
                "name": t.name,
...
fn call_tool(name: &str, args: &Value, root: &std::path::Path) -> String {
    macro_rules! arg {
        ($key:literal) => {
